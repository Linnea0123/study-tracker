...
import "./App.css";
import { getTasksByDate, addTask, updateTask, deleteTask } from "./firebase"; // 导入 Firebase 方法
...
function App() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      const tasks = await getTasksByDate(selectedDate); // 从 Firebase 获取任务
    ...
    fetchTasks();
  }, [selectedDate]); // 当 selectedDate 改变时重新加载任务
  ...
  const handleAddTask = async () => {
    ...
    const newTask = {
      ...
      timeSpent: 0,
      note: "",
      date: selectedDate, // 为任务添加日期
    };
    await addTask(newTask); // 使用 Firebase 添加任务
    setTasksByDate((prev) => {
      const copy = { ...prev };
  ...
  const handleImportTasks = async () => {
    ...
    for (const task of newTasks) {
      await addTask(task); // 使用 Firebase 添加任务
    }
    setTasksByDate((prev) => {
      const copy = { ...prev };
      if (!copy[selectedDate]) copy[selectedDate] = [];
  ...
  const toggleDone = async (task) => {
    ...
    await updateTask(task.id, updatedTask); // 使用 Firebase 更新任务
    setTasksByDate((prev) => {
      const copy = { ...prev };
  ...
  const deleteTaskHandler = async (task) => {
    await deleteTask(task.id); // 使用 Firebase 删除任务
    setTasksByDate((prev) => {
      const copy = { ...prev };
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: 15, fontFamily: "sans-serif", backgroundColor: "#f5faff" }}>
      <h1 style={{ textAlign: "center", color: "#1a73e8", fontSize: 20 }}>📚 汤1圆学习计划和打卡</h1>
